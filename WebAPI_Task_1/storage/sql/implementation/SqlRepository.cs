...
namespace WebAPI_Task_1.storage.sql.implementation;
...
public class SqlRepository<TU>: AbstractRepository, IRepository where TU : DbConnection
{
    ...
    public Task<List<T>> LoadAllFromStorage<T>( T entity, string condition = "" ) where T : AbstractEntity
    {
        return Task.Run( () =>
        {
            ...
            sqlCommand.CommandText =
                $"SELECT {SqlHelper.ConvertFieldsToString( fields )} FROM {SqlHelper.GetTableName( entity )} {condition}";
    ...
    public Task<T ?> LoadFromStorage<T>( T entity ) where T : AbstractEntity
    {
        return Task.Run( () =>
        {
            ...
            sqlCommand.CommandText =
                $"SELECT {SqlHelper.ConvertFieldsToString( fields )} FROM {SqlHelper.GetTableName( entity )} WHERE UUID = @uuid";
    ...
    public Task<int> CreateInStorage( AbstractEntity entity )
    {
        return Task.Run( () =>
        {
            try
            {
                ...
                fields.ForEach( field => stringBuilder.Append( field.FieldValue ).Append( ", " ) );
                
                sqlCommand.CommandText =
                    $"INSERT INTO {SqlHelper.GetTableName( entity )} VALUES ({stringBuilder.ToString()[..(stringBuilder.Length - 2)]})";
